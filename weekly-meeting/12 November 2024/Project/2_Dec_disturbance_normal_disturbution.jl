...
# Define disturbance grid
num_disturbance = 5  # Number of disturbance values to consider
disturbance_std = 0.1  # Standard deviation of the normal disturbance
c_dis = 5
disturbance_grid = range(-c_dis * disturbance_std, c_dis * disturbance_std, length=num_disturbance)
    ...
    for (xi, x) in enumerate(x_grid)
        for (yi, y) in enumerate(y_grid)
            for (ti, theta) in enumerate(theta_grid)
                state = (x, y, theta)
                min_value = Inf  # Correct initialization for minimization over disturbances
                best_action = (NaN, NaN)
                
                # Loop over disturbances (minimization)
                for disturbance in disturbance_grid
                    max_future_value = -Inf  # Correct initialization for maximization over actions
                    action_for_current_disturbance = (NaN, NaN)
                    ...
                    for omega in omega_grid
                        for v in v_grid
                            ...
                            # Update max_future_value and action_for_current_disturbance if future_value is greater
                            if future_value > max_future_value
                                max_future_value = future_value
                                action_for_current_disturbance = action
                            end
                        end
                    end
                    
                    # Update min_value and best_action if max_future_value is less
                    if max_future_value < min_value
                        min_value = max_future_value
                        best_action = action_for_current_disturbance
                    end
                end
...
function run_simulation_with_value_tracking(
    ...
    for k in 1:time_steps
        ...
        # Apply disturbance (simulate worst-case disturbance)
        # For simulation, select the disturbance that minimizes the future value
        best_V = Inf
        worst_disturbance = disturbance_grid[1]

        for disturbance in disturbance_grid
            # Apply dynamics to get next state with disturbance
            next_state = dynamics(current_state, action, disturbance, delta_t)
            x_next, y_next, theta_next = next_state

            # Find the closest grid indices for next state
            x_next_idx = argmin(abs.(x_grid .- x_next))
            y_next_idx = argmin(abs.(y_grid .- y_next))
            theta_next_idx = argmin(abs.(theta_grid .- theta_next))

            if 1 ≤ x_next_idx ≤ num_x && 1 ≤ y_next_idx ≤ num_y && 1 ≤ theta_next_idx ≤ num_theta
                V_next = V[k + 1, x_next_idx, y_next_idx, theta_next_idx]
            else
                V_next = -Inf  # Penalty for out-of-bounds transitions
            end
            ...
            # Update best_V and worst_disturbance if future_value is less
            if future_value < best_V
                best_V = future_value
        ...
        push!(disturbance_over_time, worst_disturbance)

        # Apply dynamics with worst disturbance to get next state
        next_state = dynamics(current_state, action, worst_disturbance, delta_t)
        x_next, y_next, theta_next = next_state
