...
μ_disturbance = 0.0  # Mean of disturbance
σ_disturbance = 0.1  # Standard deviation of disturbance
c_dis = 3  # Number of standard deviations to consider
num_disturbance = 5  # Number of disturbance values to consider in the grid
disturbance_values = range(μ_disturbance - c_dis * σ_disturbance, μ_disturbance + c_dis * σ_disturbance, length=num_disturbance)
    ...
    for (xi, x) in enumerate(x_grid)
        for (yi, y) in enumerate(y_grid)
            for (ti, theta) in enumerate(theta_grid)
                state = (x, y, theta)
                min_value = Inf  # Initialization for minimization over disturbances
                best_action = (NaN, NaN)
                
                # Loop over disturbances (minimization)
                for disturbance in disturbance_values
                    max_future_value = -Inf  # Initialization for maximization over actions
...
# Visualize Disturbance Distribution in 3D Plot
# Generate a grid over x and y
x_vals = range(lower_x, upper_x, length=50)
y_vals = range(lower_y, upper_y, length=50)
Z = zeros(length(x_vals), length(y_vals))

# For each point, compute the disturbance distribution (probability density function at zero)
for (i, x) in enumerate(x_vals)
    for (j, y) in enumerate(y_vals)
        # Compute the PDF of the disturbance at zero (as an example)
        Z[i, j] = pdf(Normal(μ_disturbance, σ_disturbance), 0.0)
    end
end
...
surface(
    ...
    xlabel = "x",
    ylabel = "y",
    zlabel = "Disturbance PDF at 0",
    title = "Disturbance Distribution over x-y Plane"
)

savefig("disturbance_distribution_3d.png")
display(current())
